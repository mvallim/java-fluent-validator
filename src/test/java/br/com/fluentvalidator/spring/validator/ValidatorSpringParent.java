...
public class ValidatorSpringParent extends AbstractValidator<Parent> {
  ...
  public void rules() {
    ...
    ruleForEach(Parent::getChildren).must(not(nullValue()))
        ...
        .whenever(not(nullValue())).withValidator(validatorChild)
        .critical(ValidationSampleException.class);

    ruleFor(Parent::getId).whenever(not(nullValue())).withValidator(validatorId)
    ...
    ruleForEach(parent -> extractBoys(parent.getChildren()))
        ...
        .withValidator(validatorBoy).critical();
