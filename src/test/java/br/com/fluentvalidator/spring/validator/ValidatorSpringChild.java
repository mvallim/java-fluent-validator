...
public class ValidatorSpringChild extends AbstractValidator<Child> {
  ...
  protected void rules() {
    ...
    ruleFor(Child::getAge).must(not(nullValue())).withMessage("child age must be not null")
        .withFieldName("age").critical().must(greaterThanOrEqual(5)).when(not(nullValue()))
        .withMessage("child age must be greater than or equal to 5").withFieldName("age")
        .must(this::checkAgeConstraintChild).when(not(nullValue()))
        .withMessage("child age must be less than age parent").withFieldName("age").critical();

    ruleFor(Child::getName).must(not(stringEmptyOrNull()))
        .withMessage("child name must be not null or empty").withFieldName("name");
